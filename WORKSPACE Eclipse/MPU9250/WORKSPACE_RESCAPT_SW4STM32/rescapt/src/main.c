    ...
    while (1) {
     ...
     VL6180x_Step();
      ...
      HAL_Delay(10);
      	...
      	txMsg.id=0x11;
		txMsg.data[0] = (acc[0] & 0xFF);
		txMsg.data[1] = (acc[0]>>8 & 0xFF);
		txMsg.data[2] = (acc[1] & 0xFF);
		txMsg.data[3] = (acc[1]>>8 & 0xFF);
		txMsg.data[4] = (acc[2] & 0xFF);
		txMsg.data[5] = (acc[2]>>8 & 0xFF);
		txMsg.data[6] = 0;
		txMsg.data[7] = 241; 				// acc response -> 241
		txMsg.len=8;
		...
		txMsg.id=0x11;
		txMsg.data[0] = (gyr[0] & 0xFF);
		txMsg.data[1] = (gyr[0]>>8 & 0xFF);
		txMsg.data[2] = (gyr[1] & 0xFF);
		txMsg.data[3] = (gyr[1]>>8 & 0xFF);
		txMsg.data[4] = (gyr[2] & 0xFF);
		txMsg.data[5] = (gyr[2]>>8 & 0xFF);
		txMsg.data[6] = 0;
		txMsg.data[7] = 242; 				// gyr response -> 242
		txMsg.len=8;
		txMsg.format=CANStandard;
		txMsg.type=CANData;
...
void can_callback(void)
{
	...
	can_Read(&msg_rcv);
	txMsg.id=0x11;			// Identifiant du message à envoyer
