...
int dxl_readPacket()
{
	...
	switch(state)
	{
		...
		case 	STATE_ID : 			id = tmp ; state = STATE_LENGTH_LB;
								//term_printf("id = %d | ",id);
								break;
		...
		case 	STATE_PARAM : if( length > 0 ){
								...
								if (length == 0) {
									state = STATE_CRC1;
																					...
																					break;
		case	STATE_CRC1 : state = STATE_CRC2;
				//term_printf("CRC1 = 0x%x | ",tmp);
				break;
		case 	STATE_CRC2 : state = STATE_FINAL;
				//term_printf("CRC2 = 0x%x ",tmp);
				break;
